...
class ProgressIntegrationTest(private val level: Level) {
    ...
    @Test(timeout = 3000)
    fun `solving exercises increases level progress correctly`() {
        runBlocking {
            val activeUserId = 1L
            ...
            withClue("Level ${level.id}: Progress $levelProgress should be close to $expectedProgress (Solved $exercisesToSolve/$totalFacts)") {
                levelProgress shouldBe (expectedProgress plusOrMinus tolerance)
            }
        }
    }
}
